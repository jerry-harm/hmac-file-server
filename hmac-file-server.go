...
type Config struct {
    ...
    MaxUploadSize          int64  `toml:"max_upload_size"` // Max upload size in bytes
}
...
var conf = Config{
    ...
    MaxRetries: 5,
    RetryDelay: 2,
    MaxUploadSize: 1073741824, // Default to 1 GB
}
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
    ...
    if r.Method == http.MethodPut {
        ...
        if int64(len(data)) > conf.MaxUploadSize {
            log.Warnf("Uploaded file exceeds maximum size: %d bytes", conf.MaxUploadSize)
